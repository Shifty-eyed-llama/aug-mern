...
function findObjectsFilter(searchFor, items) {
    ...
    for (let i = 0; i < keys.length; i++) {
        if (items.hasOwnProperty(keys[i]) && items[keys[i]] === searchFor[keys[i]]) {
...
function SearchFor(searchFor, items) {
    return items.filter(item => item.firstName == searchFor.firstName && item.age == searchFor.age)
}

...
// The length of the array is n and the largest number is no larger than n.
...
function findMissingValue(arr) {
    let arrSum = 0; // one sum for the array
    let realSum = arr.length; // one sum for the actual total

    for (var i = 0; i < arr.length; i++) {
        arrSum += arr[i]; // sum the values in the array
        realSum += i; // sum the values that should be there
    }
...
// You are given an an array (up to 255 million long) of
// integers that has first been sorted, then rotated by an unknown amount.

// Find and return the minimum value. Don't use any in-built Array functions.

// How can this be done without linearly traversing the array?

// better than 0(n)

// Binary Search

// Given [10, 11, 12, 13, 17, 18, 1, 5, 6, 9]
// return 1

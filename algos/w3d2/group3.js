  /*  Group Participants - Daniel Havens, Chris Land, Joseph Sunderland, Robert Orellana
  ...
  function getMaxServings(recipe, available) {
      let counts = [];
      for (let [ingName, amount] of Object.entries(recipe)) {
          if (available.hasOwnProperty(ingName) && available[ingName] / amount > 0) {
              counts.push(available[ingName] / amount);
          } else {
              return 0;
          }
      }
  ...
  function getMaxServings2(recipe, available) {
      let counts = Object.entries(recipe).map(([ing, amt]) => {
          if (available.hasOwnProperty(ing) && available[ing] / amt) {
              return available[ing] / amt;
          } else {
              return 0;
          }
      })
  ...
  console.log(getMaxServings2(recipe, available));